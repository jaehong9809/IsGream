...
const TestSelector: React.FC<TestSelectorProps> = ({selectedTests, onTestSelector}) => {
    return (
        <div className="w-full flex flex-wrap gap-2">
            <button 
                onClick={() => onTestSelector('HTP')}
                className={`px-3 py-2 rounded-full border border-[#009E28] text-xs 
                    ${selectedTests.find(test => test.id === 'HTP')?.isSelected 
            ...
            <button 
                ...
                className={`px-3 py-2 rounded-full border border-[#009E28] text-xs 
                    ${selectedTests.find(test => test.id === 'PAT')?.isSelected 
                        ? 'bg-[#009E28] text-white' 
                        : 'bg-white text-[#009E28]'}`}
            >
                부모양육태도 검사(PAT)
            ...
            <button 
                ...
                className={`px-3 py-2 rounded-full border border-[#009E28] text-xs 
                    ${selectedTests.find(test => test.id === 'BFI')?.isSelected 
                        ? 'bg-[#009E28] text-white' 
                        : 'bg-white text-[#009E28]'}`}
            >
                성격 5요인 검사(BFI)
