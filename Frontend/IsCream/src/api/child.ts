...
export const childApi = {
  ...
  async addChild(childData: { nickname: string, gender: string, birthDate: string }): Promise<Child> {
    try {
  ...
  async updateChild( childId: number, nickname: string, gender: string, birthDate: string ): Promise<Child> {
    try {
      ...
      throw new Error(response.data.message || "자녀 정보 수정에 실패했습니다.");
